...
class GachonLPSolver:
    ...
    def set_objective_variables(self, objective_coefficient_vector, optimize_setting):
        self._objective_variables = objective_coefficient_vector
    ...
    def add_constraints(self, constraints_coefficient_vector, sign, rhs):
        ...
        temp = constraints_coefficient_vector + [rhs]
        temp = np.array(temp)

        if self._constraints_coefficient_matrix is None:
            self._constraints_coefficient_matrix = temp
        else:
            self._constraints_coefficient_matrix = np.vstack((self._constraints_coefficient_matrix,temp))

        # print(self._constraints_coefficient_matrix)
